 ...
 */
class MultiLineStringHandler extends GWTOLGeometryHandler {

    public MultiLineStringHandler() {
        super.setSuccessor(new LinearRingHandler());
    }
    ...
    public Geometry buildGeometry(String geomClassName, JSObject geomElement)
        ...
        return (isCompatibleGeometry(geomClassName)
                ? MultiLineString.narrowToMultiLineString(geomElement)
                : super.forwardBuildGeometry(geomClassName, geomElement));
    }

    @Override
    protected boolean isCompatibleGeometry(String geomClassName) {
        return geomClassName.equals(Geometry.MULTI_LINE_STRING_CLASS_NAME);
    }
