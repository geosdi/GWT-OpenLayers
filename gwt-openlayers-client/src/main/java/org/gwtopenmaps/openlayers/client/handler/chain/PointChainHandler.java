 ...
 */
public class PointChainHandler extends ChainHandler {

    public PointChainHandler() {
        super.setSuccessor(new PathChainHandler());
    }
    ...
    public <H extends Handler> H buildHandler(JSObject jsObject) {
        return (H) ((super.canBuildHandler(jsObject.getPropertyAsString(
                "CLASS_NAME")))
                ? PointHandler.narrowToHandler(jsObject)
                : super.forwardBuildHandler(jsObject));
    }

    @Override
    protected String getClassHandlerType() {
        return Handler.POINT_HANDLER_CLASS_NAME;
    }
