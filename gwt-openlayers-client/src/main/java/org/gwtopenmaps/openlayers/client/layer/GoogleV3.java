...
public class GoogleV3 extends Layer {
    ...
    public LonLat forwardMercator(double lon, double lat) {
        return LonLat.narrowToLonLat(GoogleV3Impl.forwardMercator(getJSObject(),
                lon, lat));
    }

    @Override
    protected LayerCreator<GoogleV3> getLayerCreator() {
        return new LayerCreator<GoogleV3>() {

            public GoogleV3 createLayer(JSObject jsObject) {
