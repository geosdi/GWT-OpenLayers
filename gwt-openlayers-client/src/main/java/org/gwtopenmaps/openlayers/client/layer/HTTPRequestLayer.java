...
/**
 * Wrapper of OpenLayers.Layer.HTTPRequest in OpenLayers. Renamed HTTPREquest to
 * HTTPRequestLayer so it is not confused with HTTPRequest in GWT.
 * 
...
public class HTTPRequestLayer extends Layer {
     ...
     *
     * Parameters: name - {String} url - {String} params - {WMSParams} options -
     * {HTTPRequestLayerOptions} Hashtable of extra options to tag onto the
     * layer
     */
     ...
     * Constructor: HTTPRequestLayer
     *
     * Parameters: name - {String} url - {String} params - {WMSParams}
     *
     */
    public HTTPRequestLayer(String name, String url, WMSParams params) {
        this(HTTPRequestLayerImpl.create(name, url, params.getJSObject()));
     ...
     * options - {HTTPRequestLayerOptions} Hashtable of extra options to tag
     * onto the layer
     */
     ...
     * APIMethod: clone
     *
     * Parameters: obj - {HTTPRequest}
     *
     * Returns: {HTTPRequest} An exact clone of this HTTPRequest
     */
    public HTTPRequestLayer clone(HTTPRequestLayer obj) {
        return new HTTPRequestLayer(HTTPRequestLayerImpl.Clone(getJSObject(),
                obj.getJSObject()));
    }
    ...
    public boolean mergeNewParams(Params params) {
        return HTTPRequestLayerImpl.mergeNewParams(getJSObject(),
                params.getJSObject());
    }

    /**
     * @param force - if true force redraw by adding random parameter to getMap
     * request
     *
    ...
    /**
     * Method: selectUrl selectUrl() implements the standard floating-point
     * multiplicative hash function described by Knuth, and hashes the contents
     * of the given param string into a float between 0 and 1. This float is
     * then scaled to the size of the provided urls array, and used to select a
     * URL.
     *
     * Parameters: paramString - {String} urls - {String}
     *
     * Returns: {String} An entry from the urls array, deterministically
     * selected based on the paramString.
     */
...
//    }
    /**
     * Method: getFullRequestString Combine url with layer's params and these
     * newParams.
     *
     * does checking on the serverPath variable, allowing for cases when it is
     * supplied with trailing ? or &, as well as cases where not.
     *
     ...
     *
     * Parameters: newParams - {WMSParams} altUrl - {String} Use this as the url
     * instead of the layer's url
     *
     * Returns: {String}
     */
    public String getFullRequestString(WMSParams newParams, String altUrl) {
        return HTTPRequestLayerImpl.getFullRequestString(getJSObject(),
                newParams.getJSObject(), altUrl);
    }
    ...
    protected <L extends Layer> LayerCreator<L> getLayerCreator() {
        return new LayerCreator<L>() {
            ...
            public L createLayer(JSObject jsObject) {
                return (L) new HTTPRequestLayer(jsObject);
            }
        };
    }
