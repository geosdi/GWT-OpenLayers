...
/**
 * For displaying FeatureCollection data requested from a Web Feature Service
 * (WFS) server.
 *
...
public class WFS extends Vector {
    ...
    public WFS(String name, String url, WFSParams params, WFSOptions options) {
        this(WFSImpl.create(name, url, params.getJSObject(),
                options.getJSObject()));
    }

    @Override
    protected LayerCreator<WFS> getLayerCreator() {
        return new LayerCreator<WFS>() {
