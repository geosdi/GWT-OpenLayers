...
public class SelectFeatureExample extends AbstractExample
{
    ...
    private final RadioButton rbDrawPolygon = new RadioButton("grp", "draw polygon");
    private final RadioButton rbSelectOnHover = new RadioButton("grp", "select features on hover");
    private final RadioButton rbSelectFeatures = new RadioButton("grp", "select feature (0 features selected)");
    private final RadioButton rbSelectBox = new RadioButton("grp", "select features in a box");
    private final CheckBox cbClickOut = new CheckBox("click out to unselect features");
     ...
     */
    public SelectFeatureExample(String title, TextResource source)
    {
        super(title, source);
    }
    ...
    public void buildPanel()
    {
        ...
        vectorLayer.addVectorFeatureSelectedListener(new VectorFeatureSelectedListener() //update label that shows the number of selected features
        {
            ...
            {
                int length = 0;
                if (vectorLayer.getSelectedFeatures() != null) length = vectorLayer.getSelectedFeatures().length;
                rbSelectFeatures.setText("select feature (" + length + " features selected)");
            }
        });
        ...
        drawPointFeatureControl = new DrawFeature(vectorLayer, new PointHandler());
        ...
        drawPolygonFeatureControl = new DrawFeature(vectorLayer, new PolygonHandler());
        ...
        boxSelectFeature = new SelectFeature(vectorLayer, boxSelectFeatureOptions);
        ...
        hoverSelectFeature = new SelectFeature(vectorLayer, hoverSelectFeatureOptions);
    ...
    {
        if (rbDrawPoint.getValue()) drawPointFeatureControl.activate();
        else drawPointFeatureControl.deactivate();

        if (rbDrawLine.getValue()) drawLineFeatureControl.activate();
        else drawLineFeatureControl.deactivate();

        if (rbDrawPolygon.getValue()) drawPolygonFeatureControl.activate();
        else drawPolygonFeatureControl.deactivate();

        if (rbSelectFeatures.getValue()) clickSelectFeature.activate();
        else clickSelectFeature.deactivate();

        if (rbSelectOnHover.getValue()) hoverSelectFeature.activate();
        else hoverSelectFeature.deactivate();

        if (rbSelectBox.getValue()) boxSelectFeature.activate();
        else boxSelectFeature.deactivate();
    }
}